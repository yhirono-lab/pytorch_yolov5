...
from utils import set_logging, colorstr, select_device, increment_path, check_file, methods,\
    ...
    check_img_size, labels_to_class_weights, EarlyStopping, strip_optimizer, de_parallel
from loss import ComputeLoss, fitness
from plot_utils import plot_labels, on_fit_epoch_end, on_train_end
...
def train(hyp, opt, device, callbacks=Callbacks()):
    ...
    # Image sizes(32の倍数であるか確認する)
    gs = max(int(model.stride.max()), 32)  # grid size (max stride)
    nl = model.head.detect.nl  # number of detection layers (used for scaling hyp['obj'])
    ...
    if RANK in [-1, 0]:
        val_loader = create_dataloader(val_path, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls,
                                       hyp=hyp, rect=True, rank=-1, workers=workers, pad=0.5, prefix=colorstr('val: '))[0]
    ...
    hyp['obj'] *= (imgsz / 224) ** 2 * 3. / nl  # scale to image size and layers
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            on_fit_epoch_end(log_vals, epoch, best_fitness, fi, save_dir)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default='data/Hodgkin/Hodgkin.yaml', help='dataset.yaml path')
